...
int main(int argc, char *argv[])
{
    ...
    string text_file = "../pizzachili/englishV2.1024MB";
    std::string pattern = "the";
    csa_wt<> fm;

    construct(fm, text_file, 1);
    executed_runs = 0;
    for (n = lower; n <= upper; n += step) {
        ...
        //size_t pos = rand() % (text_file.size() - n);
        //std::string pattern = text_file.substr(pos , n);


        // Run to compute elapsed time
        DocumentSuffixArray sa_index("../corpus_proteins.txt");
        cout << "Tamaño del sa-index " << sa_index.size_in_mb() << " MB." << endl;
        ...
        for (i = 0; i < runs; i++) {
            ...
            sa_index.doc_locate("<?xml");
            //DocumentFMIndex index("../corpus.txt");

            
            end_time = std::chrono::high_resolution_clock::now();
            
            
            //node* x = fh.Create_node(rand() % n);
            //H = fh.Insert(H, x);
